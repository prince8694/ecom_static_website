...
def home(request,c_slug=None):
    ...
    if c_slug!=None:
        ...
        prdt=products.objects.filter(category=c_page,available=True)
    else:
        prdt=products.objects.all().filter(available=True)
    ...
    return render(request,'home.html',{'pr':prdt,'ct':cat,'pg':pro})
